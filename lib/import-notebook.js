...
export function ipynbOpener(uri) {
  if (
    ...
    atom.config.get("hydrogen-next.importNotebookURI") === true
  ) {
    return _loadNotebook(uri, atom.config.get("hydrogen-next.importNotebookResults"))
  }
}
...
export function importNotebook(event) {
  ...
  if (filenameFromTreeView && path.extname(filenameFromTreeView) === ".ipynb") {
    return _loadNotebook(
      ...
      atom.config.get("hydrogen-next.importNotebookResults")
    )
  }
  ...
  dialog.showOpenDialog(
    ...
    filenames => {
      ...
      _loadNotebook(filename, atom.config.get("hydrogen-next.importNotebookResults"))
    }
  )
}
 ...
 */
export async function _loadNotebook(filename, importResults = false) {
  let data
  let nb
